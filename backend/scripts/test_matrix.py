"""Comprehensive positive and negative control matrix for verifying deterministic router actions outside of pytest."""
...
async def run_evaluation_matrix():
    ...
    test_cases = [
        # --- POSITIVE CONTROLS (Allowed, Clean, 0 False Positives) ---
        {
            "id": "POS-01",
        ...
        },
        {
            ...
            "payload": {
                ...
                "token_count": 280,
                "metadata": {
                    "asset_id": "test_neg_spike",
                    "valuation_amount": 580000.0,
                    "baseline_mean": 400000.0,
                    "baseline_std": 20000.0,
                },
            },
    ...
    print(
        f"{'ID':<8} | {'Type':<8} | {'Scenario Name':<42} | {'Expected':<12} | {'Actual':<12} | {'Latency':<8} | {'Status'}"
    )
    print("-" * 105)
    ...
    for tc in test_cases:
        ...
        print(
            f"{tc['id']:<8} | {tc['type']:<8} | {tc['name'][:40]:<42} | {tc['expected_action']:<12} | {action:<12} | {elapsed_ms:5.2f}ms  | {status_str}"
        )

    print("=" * 105)
    print(
        f"EVALUATION SUMMARY: {passed_count}/{len(test_cases)} tests passed ({passed_count/len(test_cases)*100:.1f}%)"
    )
